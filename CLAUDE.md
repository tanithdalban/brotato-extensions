...
- **Filtres tier/classe = listes déroulantes « maison », pas des `OptionButton`** : un `OptionButton` ouvre un `PopupMenu` natif **globalement modal** qui fige/parasite l'autre joueur en coop (le `FocusEmulator` sait pourtant piloter un popup via `_handle_popup_menu_input`, mais le popup natif capte n'importe quel device). Remplacés par une liste in-panel de vrais `Button` enfants du panneau (donc dans le territoire du `FocusEmulator`), rangés dans `_dropdown_overlay` (Control superposé, non-conteneur → positionné en absolu sous le champ). Confinement de la nav par **voisins de focus** (`set_focus_neighbour` : haut/bas bouclent, gauche/droite absorbés) + l'état **visible/caché** (les options cachées sont ignorées par `_filter_control`). Voir `_build_dropdown`/`_open_dropdown`/`_close_dropdown`/`_on_value_pressed`. Fermeture sur `ui_cancel` : panneau en coop, écran en solo (`is_dropdown_open()`/`close_dropdown()`). On raisonne toujours par **INDEX** (`_tier_index`/`_class_index`, 0 = « tout ») — cf. `_TIER_VALUES`, `_selected_tier()`, `_matches_class()`. Même esprit que les onglets Objets/Armes (boutons, pas le bandeau natif du `TabContainer`).
